 ...
 */
@Component
public class OnErrorAnalyzer extends WSMessageAnalyzer<IMMessageData> {
    ...
    public void analyze(WebSocketSession session, IMMessageData message) throws Throwable {
        ...
        if(CollUtil.isNotEmpty(joinRecordList)){
            List<WebSocketSession> sessionList = joinRecordList.stream()
    ...
    public AnalyzerProperties properties() {
        return new AnalyzerProperties(VedioMeetingWSHandler.class, SocketContext.RequestEventType.ERROR);
    }
}
